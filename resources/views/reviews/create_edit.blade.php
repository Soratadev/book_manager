            ...
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    <form method="POST" action="{{empty($review) ? route('reviews.store') : route('reviews.update', $review)}}">
                        ...
                        <x-text-input id="title" class="block mt-1 w-full" type="text" name="title" :value="old('title', empty($review) ? '' : $review->title)" placeholder="título del libro" />
                        ...
                        <div class="mt-4 space-x-8">
                            <x-primary-button>{{empty($review) ? 'Guardar' : 'Actualizar' }}</x-primary-button>
                            <a href="{{route('reviews.index')}}" class="dark:text-gray-100">{{ __('Cancelar') }}</a>
                        </div>
                    </form>
                </div>
            </div>
